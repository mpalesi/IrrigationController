...
static const char PAGE[] =
    ...
    "async function loadPrograms(){config=await (await fetch('/programs')).json();programs.innerHTML='';config.programs.forEach(card);showZoneNames()}"
...
typedef struct {
    ...
    char zone_name[DEV_KIT_ZONE_NAME_SIZE];
...
static const char *zone_name_for(const DevKitWebContext *c, const char *id) { int index = zone_index_for(c, id); const char *name = index >= 0 ? dev_kit_zone_configuration_name(c->zone_configuration, (size_t)index) : NULL; return name != NULL ? name : id; }
...
static esp_err_t programs_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; size_t used = (size_t)snprintf(programs_response, sizeof(programs_response), "{\"zones\":["); for (size_t i = 0; i < c->zone_count; ++i) used += (size_t)snprintf(programs_response + used, sizeof(programs_response) - used, "%s{\"id\":\"%s\",\"name\":\"%s\"}", i ? "," : "", c->zones[i].id, zone_name_for(c, c->zones[i].id)); used += (size_t)snprintf(programs_response + used, sizeof(programs_response) - used, "],\"programs\":["); bool first = true; for (size_t i = 0; i < DEV_KIT_WEB_MAX_PROGRAMS; ++i) { DevKitProgramConfiguration *d = c->program_configuration; if (!d->in_use[i]) continue; Program *p = &d->programs[i]; used += (size_t)snprintf(programs_response + used, sizeof(programs_response) - used, "%s{\"index\":%u,\"name\":\"%s\",\"total_duration_ms\":%u,\"steps\":[", first ? "" : ",", (unsigned)i, p->id, (unsigned)total_ms(p)); for (size_t j = 0; j < p->step_count; ++j) used += (size_t)snprintf(programs_response + used, sizeof(programs_response) - used, "%s{\"zone_id\":\"%s\",\"duration_ms\":%u}", j ? "," : "", p->steps[j].zone_id, (unsigned)p->steps[j].duration_ms); used += (size_t)snprintf(programs_response + used, sizeof(programs_response) - used, "]}"); first = false; } snprintf(programs_response + used, sizeof(programs_response) - used, "]}"); httpd_resp_set_type(r, "application/json"); return httpd_resp_sendstr(r, programs_response); }
...
static esp_err_t zone_name_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; if (!query_value(r, "id", request_scratch.zone_id, sizeof(request_scratch.zone_id)) || !query_value(r, "name", request_scratch.zone_name, sizeof(request_scratch.zone_name))) return send_result(r, IRRIGATION_RESULT_REJECTED); int index = zone_index_for(c, request_scratch.zone_id); return send_result(r, index < 0 ? IRRIGATION_RESULT_NOT_FOUND : dev_kit_zone_configuration_set_name(c->zone_configuration, (size_t)index, request_scratch.zone_name)); }
