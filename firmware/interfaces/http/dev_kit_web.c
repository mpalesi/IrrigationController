...
static IrrigationResult configure_scheduler(DevKitWebContext *context, IrrigationResult result) { return result == IRRIGATION_RESULT_OK ? configuration_manager_configure_scheduler(context->configuration_manager, context->scheduler_service) : result; }
