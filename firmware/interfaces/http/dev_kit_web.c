...
#define SCHEDULE_ID_SIZE 32U
#define MAX_STEP_SECONDS 3600U
#define URI_HANDLER_COUNT 15U
...
static const char PAGE[] =
    ...
    "<script>let config;const programs=document.querySelector('#programs'),scheduleEditor=document.querySelector('#schedule'),statusOutput=document.getElementById('status');"
    "async function go(u){let r=await fetch(u,{method:'POST'}),text=await r.text();alert(text);load();if(text.includes('\"result\":\"OK\"')&&(u.startsWith('/program/save')||u.startsWith('/program/delete')))loadPrograms();if(text.includes('\"result\":\"OK\"')&&(u.startsWith('/schedule/save')||u.startsWith('/schedule/delete')||u.startsWith('/schedule/toggle')))loadSchedule()}"
...
static SchedulerEntry schedule_entries[IRRIGATION_MAX_SCHEDULES];
static char schedule_ids[IRRIGATION_MAX_SCHEDULES][SCHEDULE_ID_SIZE];
static uint32_t next_schedule_id;
...
static int program_index_for(const DevKitWebContext *c, const Program *program) { for (size_t i = 0U; i < DEV_KIT_WEB_MAX_PROGRAMS; ++i) if (c->program_configuration->in_use[i] && &c->program_configuration->programs[i] == program) return (int)i; return -1; }
static void copy_schedule_entries(const SchedulerService *service) { for (size_t i = 0U; i < service->entry_count; ++i) { schedule_entries[i] = service->entries[i]; (void)snprintf(schedule_ids[i], sizeof(schedule_ids[i]), "%s", service->entries[i].id); schedule_entries[i].id = schedule_ids[i]; } }
...
static esp_err_t schedule_get_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; size_t used = (size_t)snprintf(schedule_response, sizeof(schedule_response), "{\"schedules\":["); for (size_t i = 0U; i < c->scheduler_service->entry_count; ++i) { const SchedulerEntry *entry = &c->scheduler_service->entries[i]; int program_index = program_index_for(c, entry->program); if (program_index < 0) return send_result(r, IRRIGATION_RESULT_NOT_FOUND); used += (size_t)snprintf(schedule_response + used, sizeof(schedule_response) - used, "%s{\"index\":%u,\"weekday_mask\":%u,\"hour\":%u,\"minute\":%u,\"program_index\":%d,\"enabled\":%s}", i ? "," : "", (unsigned)i, (unsigned)entry->weekday_mask, (unsigned)entry->hour, (unsigned)entry->minute, program_index, entry->enabled ? "true" : "false"); } (void)snprintf(schedule_response + used, sizeof(schedule_response) - used, "]}"); httpd_resp_set_type(r, "application/json"); return httpd_resp_sendstr(r, schedule_response); }
...
static esp_err_t schedule_save_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; unsigned long index, weekday_mask, hour, minute, program_index; if (!query_value(r, "index", request_scratch.schedule_index, sizeof(request_scratch.schedule_index))) return send_result(r, IRRIGATION_RESULT_REJECTED); const bool is_new = strcmp(request_scratch.schedule_index, "new") == 0; if (!is_new && !parse_value(request_scratch.schedule_index, IRRIGATION_MAX_SCHEDULES - 1U, &index)) return send_result(r, IRRIGATION_RESULT_REJECTED); if ((!is_new && index >= c->scheduler_service->entry_count) || !query_value(r, "weekday_mask", request_scratch.schedule_weekday_mask, sizeof(request_scratch.schedule_weekday_mask)) || !query_value(r, "hour", request_scratch.schedule_hour, sizeof(request_scratch.schedule_hour)) || !query_value(r, "minute", request_scratch.schedule_minute, sizeof(request_scratch.schedule_minute)) || !query_value(r, "program_index", request_scratch.schedule_program_index, sizeof(request_scratch.schedule_program_index)) || !parse_value(request_scratch.schedule_weekday_mask, (1U << 7U) - 1U, &weekday_mask) || weekday_mask == 0U || !parse_value(request_scratch.schedule_hour, 23U, &hour) || !parse_value(request_scratch.schedule_minute, 59U, &minute) || !parse_value(request_scratch.schedule_program_index, DEV_KIT_WEB_MAX_PROGRAMS - 1U, &program_index) || !slot_valid(c, program_index)) return send_result(r, IRRIGATION_RESULT_REJECTED); size_t count = c->scheduler_service->entry_count; copy_schedule_entries(c->scheduler_service); if (is_new) { if (count >= IRRIGATION_MAX_SCHEDULES) return send_result(r, IRRIGATION_RESULT_REJECTED); index = count++; (void)snprintf(schedule_ids[index], sizeof(schedule_ids[index]), "dev-schedule-%u", (unsigned)next_schedule_id++); schedule_entries[index] = (SchedulerEntry){.id = schedule_ids[index]}; } schedule_entries[index].weekday_mask = (uint8_t)weekday_mask; schedule_entries[index].hour = (uint8_t)hour; schedule_entries[index].minute = (uint8_t)minute; schedule_entries[index].program = &c->program_configuration->programs[program_index]; return send_result(r, scheduler_service_configure(c->scheduler_service, schedule_entries, count)); }
static esp_err_t schedule_delete_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; unsigned long index; if (!query_value(r, "index", request_scratch.schedule_index, sizeof(request_scratch.schedule_index)) || !parse_value(request_scratch.schedule_index, IRRIGATION_MAX_SCHEDULES - 1U, &index)) return send_result(r, IRRIGATION_RESULT_REJECTED); if (index >= c->scheduler_service->entry_count) return send_result(r, IRRIGATION_RESULT_NOT_FOUND); size_t count = 0U; for (size_t source = 0U; source < c->scheduler_service->entry_count; ++source) { if (source == index) continue; schedule_entries[count] = c->scheduler_service->entries[source]; (void)snprintf(schedule_ids[count], sizeof(schedule_ids[count]), "%s", schedule_entries[count].id); schedule_entries[count].id = schedule_ids[count]; ++count; } return send_result(r, scheduler_service_configure(c->scheduler_service, schedule_entries, count)); }
static esp_err_t schedule_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; unsigned long index; if (!query_value(r, "index", request_scratch.schedule_index, sizeof(request_scratch.schedule_index)) || !parse_value(request_scratch.schedule_index, IRRIGATION_MAX_SCHEDULES - 1U, &index)) return send_result(r, IRRIGATION_RESULT_REJECTED); if (index >= c->scheduler_service->entry_count) return send_result(r, IRRIGATION_RESULT_NOT_FOUND); return send_result(r, scheduler_service_set_enabled(c->scheduler_service, index, !c->scheduler_service->entries[index].enabled)); }

void dev_kit_web_start(const DevKitWebContext *context) { if (server != NULL || context == NULL) return; httpd_config_t config = HTTPD_DEFAULT_CONFIG(); config.max_uri_handlers = URI_HANDLER_COUNT; if (httpd_start(&server, &config) != ESP_OK) return; const httpd_uri_t handlers[] = {{.uri="/",.method=HTTP_GET,.handler=page_handler,.user_ctx=(void *)context},{.uri="/status",.method=HTTP_GET,.handler=status_handler,.user_ctx=(void *)context},{.uri="/programs",.method=HTTP_GET,.handler=programs_handler,.user_ctx=(void *)context},{.uri="/schedule",.method=HTTP_GET,.handler=schedule_get_handler,.user_ctx=(void *)context},{.uri="/zone/open",.method=HTTP_POST,.handler=zone_handler,.user_ctx=(void *)context},{.uri="/zone/close",.method=HTTP_POST,.handler=zone_handler,.user_ctx=(void *)context},{.uri="/master/open",.method=HTTP_POST,.handler=master_handler,.user_ctx=(void *)context},{.uri="/master/close",.method=HTTP_POST,.handler=master_handler,.user_ctx=(void *)context},{.uri="/program/start",.method=HTTP_POST,.handler=program_handler,.user_ctx=(void *)context},{.uri="/program/abort",.method=HTTP_POST,.handler=program_handler,.user_ctx=(void *)context},{.uri="/program/save",.method=HTTP_POST,.handler=program_save_handler,.user_ctx=(void *)context},{.uri="/program/delete",.method=HTTP_POST,.handler=program_delete_handler,.user_ctx=(void *)context},{.uri="/schedule/save",.method=HTTP_POST,.handler=schedule_save_handler,.user_ctx=(void *)context},{.uri="/schedule/delete",.method=HTTP_POST,.handler=schedule_delete_handler,.user_ctx=(void *)context},{.uri="/schedule/toggle",.method=HTTP_POST,.handler=schedule_handler,.user_ctx=(void *)context}}; for (size_t i = 0; i < sizeof(handlers)/sizeof(handlers[0]); ++i) if (httpd_register_uri_handler(server, &handlers[i]) != ESP_OK) ESP_LOGE(TAG, "failed to register URI %s", handlers[i].uri); }
