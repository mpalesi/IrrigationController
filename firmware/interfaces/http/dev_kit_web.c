...
static const char PAGE[] =
    ...
    "<h2>Schedule</h2><div id=schedule></div>"
    ...
    "async function go(u){let r=await fetch(u,{method:'POST'}),text=await r.text();alert(text);load();if(text.includes('\"result\":\"OK\"')&&(u.startsWith('/program/save')||u.startsWith('/program/delete')))loadPrograms();if(text.includes('\"result\":\"OK\"')&&(u.startsWith('/schedule/save')||u.startsWith('/schedule/toggle')))loadSchedule()}"
    ...
    "function saveSchedule(){let q=new URLSearchParams({weekday:scheduleEditor.querySelector('.weekday').value,hour:scheduleEditor.querySelector('.hour').value,minute:scheduleEditor.querySelector('.minute').value,program_index:scheduleEditor.querySelector('.schedule-program').value});go('/schedule/save?'+q)}"
    "function showSchedule(s){scheduleEditor.innerHTML='';let day=document.createElement('select');day.className='weekday';weekdayNames.forEach((name,index)=>day.add(new Option(name,index,index===s.weekday,index===s.weekday)));let hour=document.createElement('input');hour.className='hour';hour.type='number';hour.min=0;hour.max=23;hour.value=s.hour;let minute=document.createElement('input');minute.className='minute';minute.type='number';minute.min=0;minute.max=59;minute.value=s.minute;let program=document.createElement('select');program.className='schedule-program';config.programs.forEach(p=>program.add(new Option(p.name,p.index,p.index===s.program_index,p.index===s.program_index)));let enabled=document.createElement('span');enabled.textContent='Enabled: '+(s.enabled?'Yes':'No');let toggle=document.createElement('button');toggle.textContent=s.enabled?'Disable':'Enable';toggle.onclick=()=>go('/schedule/toggle?index=0');let save=document.createElement('button');save.textContent='Save';save.onclick=saveSchedule;scheduleEditor.append('Day: ',day,' Time: ',hour,':',minute,' Program: ',program,enabled,toggle,save)}"
    ...
    "async function loadSchedule(){showSchedule(await (await fetch('/schedule')).json())}"
...
static char schedule_response[256];
static SchedulerEntry schedule_save_entry;
...
static unsigned int weekday_for(const SchedulerEntry *entry) { for (unsigned int day = 0U; day < 7U; ++day) if ((entry->weekday_mask & SCHEDULER_WEEKDAY_MASK(day)) != 0U) return day; return 0U; }
...
static esp_err_t schedule_get_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; if (c->scheduler_service->entry_count != 1U) return send_result(r, IRRIGATION_RESULT_NOT_FOUND); const SchedulerEntry *entry = &c->scheduler_service->entries[0]; int program_index = program_index_for(c, entry->program); if (program_index < 0) return send_result(r, IRRIGATION_RESULT_NOT_FOUND); (void)snprintf(schedule_response, sizeof(schedule_response), "{\"weekday\":%u,\"hour\":%u,\"minute\":%u,\"program_index\":%d,\"enabled\":%s}", weekday_for(entry), entry->hour, entry->minute, program_index, entry->enabled ? "true" : "false"); httpd_resp_set_type(r, "application/json"); return httpd_resp_sendstr(r, schedule_response); }
...
static esp_err_t schedule_save_handler(httpd_req_t *r) { DevKitWebContext *c = r->user_ctx; unsigned long weekday, hour, minute, program_index; if (c->scheduler_service->entry_count != 1U || !query_value(r, "weekday", request_scratch.schedule_weekday, sizeof(request_scratch.schedule_weekday)) || !query_value(r, "hour", request_scratch.schedule_hour, sizeof(request_scratch.schedule_hour)) || !query_value(r, "minute", request_scratch.schedule_minute, sizeof(request_scratch.schedule_minute)) || !query_value(r, "program_index", request_scratch.schedule_program_index, sizeof(request_scratch.schedule_program_index)) || !parse_value(request_scratch.schedule_weekday, 6U, &weekday) || !parse_value(request_scratch.schedule_hour, 23U, &hour) || !parse_value(request_scratch.schedule_minute, 59U, &minute) || !parse_value(request_scratch.schedule_program_index, DEV_KIT_WEB_MAX_PROGRAMS - 1U, &program_index) || !slot_valid(c, program_index)) return send_result(r, IRRIGATION_RESULT_REJECTED); schedule_save_entry = c->scheduler_service->entries[0]; schedule_save_entry.weekday_mask = (uint8_t)SCHEDULER_WEEKDAY_MASK(weekday); schedule_save_entry.hour = (uint8_t)hour; schedule_save_entry.minute = (uint8_t)minute; schedule_save_entry.program = &c->program_configuration->programs[program_index]; return send_result(r, scheduler_service_configure(c->scheduler_service, &schedule_save_entry, 1U)); }
...
void dev_kit_web_start(const DevKitWebContext *context) { if (server != NULL || context == NULL) return; httpd_config_t config = HTTPD_DEFAULT_CONFIG(); config.max_uri_handlers = URI_HANDLER_COUNT; if (httpd_start(&server, &config) != ESP_OK) return; const httpd_uri_t handlers[] = {{.uri="/",.method=HTTP_GET,.handler=page_handler,.user_ctx=(void *)context},{.uri="/status",.method=HTTP_GET,.handler=status_handler,.user_ctx=(void *)context},{.uri="/programs",.method=HTTP_GET,.handler=programs_handler,.user_ctx=(void *)context},{.uri="/schedule",.method=HTTP_GET,.handler=schedule_get_handler,.user_ctx=(void *)context},{.uri="/zone/open",.method=HTTP_POST,.handler=zone_handler,.user_ctx=(void *)context},{.uri="/zone/close",.method=HTTP_POST,.handler=zone_handler,.user_ctx=(void *)context},{.uri="/master/open",.method=HTTP_POST,.handler=master_handler,.user_ctx=(void *)context},{.uri="/master/close",.method=HTTP_POST,.handler=master_handler,.user_ctx=(void *)context},{.uri="/program/start",.method=HTTP_POST,.handler=program_handler,.user_ctx=(void *)context},{.uri="/program/abort",.method=HTTP_POST,.handler=program_handler,.user_ctx=(void *)context},{.uri="/program/save",.method=HTTP_POST,.handler=program_save_handler,.user_ctx=(void *)context},{.uri="/program/delete",.method=HTTP_POST,.handler=program_delete_handler,.user_ctx=(void *)context},{.uri="/schedule/save",.method=HTTP_POST,.handler=schedule_save_handler,.user_ctx=(void *)context},{.uri="/schedule/toggle",.method=HTTP_POST,.handler=schedule_handler,.user_ctx=(void *)context}}; for (size_t i = 0; i < sizeof(handlers)/sizeof(handlers[0]); ++i) if (httpd_register_uri_handler(server, &handlers[i]) != ESP_OK) ESP_LOGE(TAG, "failed to register URI %s", handlers[i].uri); }
